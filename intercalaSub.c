...
int intercalaSub(int TAMDADOS, char* filename, int* n_leituras, int* n_escrita, int* n_comparacoes)
{
    ...
    double notaSaiu;
    Aluno1 fimDoBloco; // registro auxilar para indicar o fim de um bloco
    fimDoBloco.nota = -1;
    ...
    while (cont < TAMDADOS && nNaMemoria < TAMMEM &&
           fread(&(vetAux[nNaMemoria].alu), sizeof(Aluno1), 1, dados) == 1)
    {
        analise1->comparacao++;
        vetAux[nNaMemoria].marc = 0;
        nNaMemoria++;
        cont++;
    ...
    fwrite(&vetAux[0].alu, sizeof(Aluno1), 1, fitas[fAtualE]);
    analise1->transferenciaE++;
    while (inicio < nNaMemoria)
    {
        analise1->comparacao+=2;
        int lido = 0;
        if (cont < TAMDADOS && (lido = fread(&(vetAux[0].alu), sizeof(Aluno1), 1, dados)) == 1)
        {
            analise1->transferenciaL++;
            cont++;
        ...
        if (estaoTodosMarcados(vetAux + inicio, nNaMemoria - inicio))
        {
            ...
            fwrite(&fimDoBloco, sizeof(Aluno1), 1, fitas[fAtualE]);
            analise1->transferenciaE++;
        ...
        fwrite(&vetAux[inicio].alu, sizeof(Aluno1), 1, fitas[fAtualE]);
        analise1->transferenciaE++;
    }
    fclose(dados);
    ...
    while (nFEAtivas > 1)
    {
        ...
        while (!processoTerminou)
        {
            ...
            for (int i = 0; i < nFEAtivas; i++){
                analise1->comparacao+=2;
                if (fread(&(vetAux[i].alu), sizeof(Aluno1), 1, fitas[fEntrada + i]) == 1)
                {
            ...
            while (inicio < nNaMemoria)
            {
                ...
                fwrite(&(vetAux[inicio].alu), sizeof(Aluno1), 1, fitas[fAtualS]);
                analise1->transferenciaE++;
                // le um novo registro se for possivel
                analise1->comparacao++;
                if (fread(&(vetAux[inicio].alu), sizeof(Aluno1), 1, fitas[fAtualE]) == 1)
                {
                    analise1->transferenciaL++;
                    ...
                    {
                        fseek(fitas[fAtualE], -sizeof(Aluno1), SEEK_CUR);
                        inicio++;
                        heapConstroi(vetAux + inicio, nNaMemoria - inicio, analise1);
                    }
                    else
            ...
            fwrite(&fimDoBloco, sizeof(Aluno1), 1, fitas[fAtualS]);
            analise1->transferenciaE++;
            ...
            for (int i = fEntrada; i < fEntrada + nFEAtivas; i++)
                fseek(fitas[i], sizeof(Aluno1), SEEK_CUR);
    ...
    *n_escrita = analise1->transferenciaE;
    *n_leituras = analise1->transferenciaL;
}